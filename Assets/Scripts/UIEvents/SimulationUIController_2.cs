...
public class SimulationUIController_2 : MonoBehaviour
{
    // Map Operations
    [SerializeField] private Button RandomMapButton;
    [SerializeField] private Toggle PlaceAgentToggle;
    [SerializeField] private Toggle PlaceResourceToggle;
    ...
    private void Start()
    {
        ...
        AgentTypeDropdown.AddOptions(new List<string> { "Sight", "Hearing", "Movement" });
        ...
        // Place key entities
        PlaceAgentToggle.onValueChanged.AddListener((isOn)=> {
            if (isOn) SimulationEvents.OnMapPlaceSelected?.Invoke("Agent");
            else EvaluateEmptySelection();
        });
        PlaceResourceToggle.onValueChanged.AddListener((isOn)=> {
            if (isOn) SimulationEvents.OnMapPlaceSelected?.Invoke("Resource");
            else EvaluateEmptySelection();
        });
    ...
    private void EvaluateEmptySelection()
    {
        // If the Toggle Group allowed switching everything off, send "None"
        if (!PlaceAgentToggle.isOn && !PlaceResourceToggle.isOn)
        {
            SimulationEvents.OnMapPlaceSelected?.Invoke("None");
        }
    }
}
